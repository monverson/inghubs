...
public class OrderServiceImpl implements OrderService {
    ...
    public Order createOrder(Order order) {
        Asset tryAsset = assetRepository.findByCustomerIdAndAssetName(order.getCustomer().getId(), TRY).orElseThrow(() ->
                new NotFoundException(CUSTOMER_ASSET_NOT_FOUND));
        ...
        if (order.getOrderSide() == Side.BUY && tryAsset.getUsableSize().compareTo(totalCost) < 0) {
            ...
            Asset asset = assetRepository.findByCustomerIdAndAssetName(order.getCustomer().getId(),
                    order.getAssetName()).orElseThrow(() -> new NotFoundException(ASSET_NOT_FOUND_FOR_SELL_ORDER));
    ...
    public void cancelOrder(Long orderId) {
        ...
        if (order.getOrderSide() == Side.BUY) {
            Asset asset = assetRepository.findByCustomerIdAndAssetName(order.getCustomer().getId(), TRY).orElseThrow(() ->
                    new NotFoundException(CUSTOMER_ASSET_NOT_FOUND));
            asset.setUsableSize(asset.getUsableSize().add(order.getPrice().multiply(order.getSize())));
        } else if (order.getOrderSide() == Side.SELL) {
            Asset sellAsset = assetRepository.findByCustomerIdAndAssetName(order.getCustomer().getId(),
                    order.getAssetName()).orElseThrow(() -> new NotFoundException(CUSTOMER_ASSET_NOT_FOUND));
